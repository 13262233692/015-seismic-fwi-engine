...
char ebcdic_to_ascii(unsigned char ebcdic_char) {
    return static_cast<char>(ebcdic_to_ascii_table[ebcdic_char]);
}

void ebcdic_to_ascii_string(const unsigned char* ebcdic_buf, size_t len, std::string& ascii_str) {
    ascii_str.resize(len);
    for (size_t i = 0; i < len; i++) {
        ascii_str[i] = static_cast<char>(ebcdic_to_ascii_table[ebcdic_buf[i]]);
    }
}

void ascii_to_ebcdic_string(const std::string& ascii_str, unsigned char* ebcdic_buf, size_t len) {
    size_t n = std::min(ascii_str.size(), len);
    for (size_t i = 0; i < n; i++) {
        ebcdic_buf[i] = ascii_to_ebcdic_table[static_cast<unsigned char>(ascii_str[i])];
    }
